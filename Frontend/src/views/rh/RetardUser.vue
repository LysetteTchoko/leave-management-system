                ...
                <table class="table-auto w-full border-collapse border-2 rounded">
                    ...
                    <tbody>
                        <tr v-for="retard in retards" :key="retard.id_retard" class="border-b hover:bg-gray-100">
                            <td class="px-4 py-2 text-center">{{ retard.employer.nom }}</td>
                            <td class="px-4 py-2 text-center">{{ retard.employer.prenom }}</td>
                            ...
                            <td class=" text-center">
                                <button @click="validerRetard(retard.id_retard)" :disabled="retard.valider_par_comite == 1"
                                 class="px-2 py-1 my-1 bg-red-500 disabled:opacity-50 text-white rounded hover:bg-red-00" >
                                    Valider
                                </button>
                            </td>
                        </tr>
...
import axios from "axios"
    ...
    const loadRetards = async (page = 1) => {
        try {
            const response = await axios.get("getRetard", {
                params: {
                    page,
                    statut: statut.value,
    ...
    const validerRetard = async (id) => {
        try {
            const response = await axios.put(`validerRetard/${id}`, {
                decision_comite: 'approuve'
            });
            result.value = 'Retard approuvé'
            await loadRetards();
        } catch (error) {
            console.error("Erreur approuvé retard :", error.response?.data || error);
        }
    }
